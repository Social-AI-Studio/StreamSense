...
def compute_acc_f1(results):
    """
    Computes accuracy and micro-F1 score from logits and one-hot labels.
    
    Args:
        results (dict): A dictionary with:
            - 'probs': Logits of shape [batch_size, num_classes]
            - 'labels': One-hot encoded labels of shape [batch_size, num_classes]

    Returns:
        tuple: (accuracy, micro_f1)
    """
    all_probs = results['probs']
    all_classes = results['labels']
    if isinstance(all_probs, np.ndarray):
        all_probs = torch.tensor(all_probs)

    if isinstance(all_classes, np.ndarray):
        all_classes = torch.tensor(all_classes)

    # Convert logits to predicted class indices
    preds = torch.argmax(all_probs, 0).cpu().numpy()  # Using the appropriate axis for argmax
    # Convert one-hot labels to actual class indices
    true_labels = torch.argmax(all_classes, 0).cpu().numpy()

    acc = accuracy_score(true_labels, preds)
    micro_f1 = f1_score(true_labels, preds, average='macro')
